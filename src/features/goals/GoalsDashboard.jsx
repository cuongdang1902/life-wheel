...
export default function GoalsDashboard({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden border shadow-2xl ${isDark
        ...
        <div className={`p-6 border-b flex justify-between items-center ${isDark ? 'border-slate-700' : 'border-slate-200'
          ...
          <div className="flex items-center gap-4">
            ...
            <div className={`w-20 h-20 rounded-full flex items-center justify-center ${isDark ? 'bg-slate-700' : 'bg-slate-100'
              }`} style={{
        ...
        {/* Period selector */}
        <div className={`px-6 py-4 border-b flex gap-2 ${isDark ? 'bg-slate-700/30 border-slate-700' : 'bg-slate-50 border-slate-200'
          }`}>
          {PERIODS.map(period => (
            <button
              key={period.value}
              onClick={() => setSelectedPeriod(period.value)}
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${selectedPeriod === period.value
            ...
            {areasWithGoals.map(area => (
              ...
              >
                ...
                >
                  ...
                  {area.progress > 0 && (
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${area.progress >= 70 ? 'bg-green-500/20 text-green-500' :
